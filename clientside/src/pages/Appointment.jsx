...
const Appointment = () => {
  ...
  const [slotIndex, setSlotIndex] = useState(0);
  const [slotTime, setSlotTime] = useState("");
  const [appointmentId, setAppointmentId] = useState(null); // ADD THIS
  const [bookingDone, setBookingDone] = useState(false); // ADD THIS
  ...
  // UPDATED: Create appointment first
  const bookAppointment = async () => {
    if (!token) {
      toast.warn("Login to book appointment");
      return navigate("/login");
    ...
    try {
      ...
      if (data.success) {
        ...
        setAppointmentId(data.appointmentId); // Get appointment ID from response
        setBookingDone(true); // Show payment form
        getDoctorsData();
      } else {
        toast.error(data.message);
      }
